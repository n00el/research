...
**Stats:** 11 replies, 41 reposts, 360 likes, 645 bookmarks, 53,679 views
...
![Agent Loop Control Flow](image-1.png)
...
![Workflows vs Agents](image-2.jpg)
...
**Orchestrator-Workers:** A central LLM dynamically breaks down tasks, delegates them to worker LLMs, and synthesizes the results. Nanobot's spawn tool and the sub-agent pattern in learn-claude-code both use this archetype.

**Evaluator-Optimizer:** A generator produces output, an evaluator provides feedback, and the loop continues until the standard is met. This fits tasks where quality standards are difficult to define precisely with code, such as translation or creative writing.
...
![Harness Components](image-4.jpg)
...
**If the Agent can't see it, it doesn't exist:** Knowledge must reside within the codebase itself. External documentation is invisible to a running Agent. Keep AGENTS.md to around 100 lines acting purely as an index, while splitting details into various docs directories to be referenced on demand.

**Encode constraints instead of documenting them:** Guidelines written in docs are easily ignored. Only constraints encoded into Linters, type systems, or CI rules are truly executable. Architectural layering should rely on custom Linters for mechanical enforcement, rather than manual review.
...
**Minimize merge friction:** Handle occasional test failures with reruns rather than blocking progress. In high-throughput environments, the cost of waiting for human review often exceeds the cost of fixing minor errors. Coding discipline hasn't disappeared; it's simply transitioned from manual review to machine-enforced constraints. Write it once, and it takes effect everywhere.

![OpenAI Observability Stack](image-5.jpg)
...
![Compression Strategies](image-7.jpg)

**Sliding Window:** Cost: Very Low. What Gets Dropped: Early context. Use Case: Short conversations.

**LLM Summary:** Cost: Medium. What Gets Dropped: Details, while preserving decisions. Use Case: Long tasks, involving key decisions.

**Tool Result Replacement:** Cost: Very Low. What Gets Dropped: Raw tool outputs. Use Case: Tool-intensive tasks.
...
![Skill Routing Accuracy](image-9.jpg)
...
![Compression Retention](image-10.jpg)
...
![Filesystem Context](image-11.jpg)
...
![Tool Design Principles](image-12.jpg)
...
**Granularity:** Good Tool maps to the Agent's goal; Bad Tool maps to an API action. Example: Good Tool uses `update_yuque_post`; Bad Tool uses `get_post + update_content + update_title`.

**Return:** Good Tool returns fields directly relevant to the next decision; Bad Tool returns the complete raw data.

**Error:** Good Tool provides structured errors containing suggestions for fixes; Bad Tool provides a generic string like "Error".

**Description:** Good Tool explains when to use and when NOT to use; Bad Tool only describes what the tool does.
...
![Advanced Tool Use](image-13.jpg)
...
![Good vs Bad Tool Design](image-15.jpg)
...
![Tool Message Isolation](image-16.jpg)
...
![Memory Architecture](image-18.jpg)
...
- Session Metadata: Content: Device, location, usage mode. Persistence: No (Session-level).
- User Memory: Content: ~33 key preference facts. Persistence: Yes (Injected every time).
- Conversation Summary: Content: ~15 lightweight summaries of recent chats. Persistence: Yes (Pre-generated).
- Current Session: Content: Sliding window of current chat. Persistence: No.
...
![Memory Consolidation](image-19.jpg)
...
![Long Task Recovery](image-20.jpg)
...
![Multi-Agent Models](image-22.jpg)
...
![Coordination Protocols](image-23.jpg)
...
![Error Amplification](image-24.jpg)
...
**Pass@k:** Meaning: At least one correct run out of k. Scenario: Exploring capability limits; run when aiming for breakthroughs.

**Pass^k:** Meaning: All k runs are correct. Scenario: Pre-launch regression testing; run on every change.
...
**Code Graders:** Typical Methods: String matching, unit tests (pass/fail), structural diffs, parameter validation. Certainty: Highest. Use Case: Tasks with explicit, correct answers.

**Model Graders:** Typical Methods: Rubric-based scoring, A/B comparisons, multi-model consensus. Certainty: Medium. Use Case: Semantic quality, style, reasoning processes.

**Human Graders:** Typical Methods: Expert spot-checks, annotation queues for calibration. Certainty: Reliable but slow. Use Case: Establishing baselines, calibrating auto-judges.
...
![Eval Infrastructure Errors](image-26.jpg)
...
- Memory splits into working, procedural, episodic, and semantic layers. A curated MEMORY.md, on-demand retrieval, and reversible consolidation mechanisms are the keys to cross-session consistency.
- Long task stability relies on externalizing state. Initializer Agents push tasks to the filesystem, and Coding Agents cycle reentrantly. Pass progress via files to escape context limits.
...
- OpenClaw implements these principles in a runnable system. Agents run stably not because of a highly complex loop, but because of engineering details like message decoupling, state externalization, layered prompts, memory consolidation, and strict security boundaries.
